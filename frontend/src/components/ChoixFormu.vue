...
function navFormu(idSalon, idForm) {
  router.push("/formulaire/" + idSalon + "&" + idForm)
}

onMounted(() => getFormulaires())
</script>
  ...
  <div class="container">
    <div>
      <v-card class="card" title="Accueil Temporaire" elevation="16">
        Dans cet accueil temporaire il est possible de choisir le formulaire à remplir (parmis les
        formulaires actifs)
      </v-card>
    </div>
    <div class="listeB">
      <button
        v-for="formu in listeForm"
        class="form-button"
        @click="navFormu(formu.salon.idSalon, formu.idForm)"
        :idSalon="formu.salon.idSalon"
      >
        Formulaire {{ formu.idForm }}
      </button>
    </div>
  </div>
</template>
